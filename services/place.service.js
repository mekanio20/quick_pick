...
class PlaceService {
  ...
  async placeLoginService(body) {
    try {
      ...
      if (!place) {
        return Response.NotFound("No information found!", []);
      }
      ...
      place.isActive = true;
      await place.save();
      const token = await Functions.generateJwt({
        id: place.id,
        role: "place",
      });
      return Response.Success("Login confirmed", {
        name: place.name,
        logo: place.logo,
        slug: place.slug,
        token,
  ...
  async placeAddMealService(body, placeId) {
    try {
      ...
      if (isExist) {
        return Response.BadRequest("Give another name value!", []);
      }
      ...
      const meal = await Models.Meals.create(body).catch((err) =>
        console.log(err)
      );
    ...
    try {
      if (files?.photo?.length > 0) {
        for (let file of files.photo) {
          const count = await Models.PlaceImages.count({
            where: { placeId: placeId },
          });
          if (count >= 6) {
            return Response.BadRequest("Your image limit is up!", []);
          }
  ...
  async placeAddPunchcardService(body, placeId) {
    try {
      ...
      if (punchcard >= 3) {
        return Response.BadRequest("Your limit is up!", []);
      }
      body.placeId = placeId;
      const [_, created] = await Models.Punchcards.findOrCreate({
        where: body,
        defaults: body,
      });
      if (!created) {
        return Response.BadRequest("Already exists!", []);
      }
      return Response.Created("Created successfully!", []);
    } catch (error) {
      throw { status: 500, type: "error", msg: error, detail: [] };
  ...
  async placeAddAccountService(placeId) {
    try {
      const placeEmail = await Models.Places.findOne({
        where: { id: placeId },
        attributes: ["id", "email"],
      });
      if (!placeEmail) {
        return Response.BadRequest("Email is not defined!", []);
      }
      const stripe_account = await Models.StripeAccounts.findOne({
        where: { placeId: placeEmail.id },
      });
      if (stripe_account) {
  ...
  async fetchPlaceMealDetailService(slug) {
    try {
      const meal = await Models.Meals.findOne({
        where: { slug: slug, isActive: true },
        attributes: {
          exclude: ["recomendo", "isActive", "createdAt", "updatedAt"],
        },
      }).catch((err) => console.log(err));
      if (!meal) {
        return Response.NotFound("No information found!", []);
    ...
    try {
      ...
      const orders = await Models.Orders.findAndCountAll({
        ...
        include: {
          ...
          attributes: {
            exclude: ["orderId", "mealId", "createdAt", "updatedAt"],
          },
          include: {
            model: Models.Meals,
      ...
      if (orders.count === 0) {
        return Response.BadRequest("Orders not found!", []);
      }
    ...
    try {
      ...
      const orders = await Models.Orders.findAndCountAll({
        where: {
          ...
          createdAt: { [Op.gte]: start_date, [Op.lte]: end_date },
        },
        attributes: { exclude: ["placeId", "updatedAt"] },
        include: {
          model: Models.OrderItems,
      ...
      if (orders.count === 0) {
        return Response.BadRequest("Order finished not found!", []);
      }
      return Response.Success("Successful!", orders);
    } catch (error) {
      throw { status: 500, type: "error", msg: error, detail: [] };
  ...
  async fetchPlaceOrderHistoryService(placeId, query) {
    try {
      ...
      if (order_history.count === 0) {
        return Response.BadRequest("Order history not found!", []);
      }
  ...
  async fetchPlaceOrderScheduleService(placeId, query) {
    try {
      ...
      if (order_schedule.count === 0) {
        return Response.BadRequest("Order schedule not found!", []);
      }
  ...
  async fetchPlaceService(slug) {
    try {
      ...
      if (images.count > 0) result.album = images;
      else result.album = {};
      ...
      if (schedule.length > 0) {
        ...
        for (let item of schedule) {
          if (day === item.day) {
            result.opening_time = [item.open_time, item.close_time];
          }
        }
      }
      ...
      const time = await meals.map((item) => item.time);
      ...
      if (time.length >= 2)
        result.times = [Math.min(...time), Math.max(...time)];
      else if (time.length === 1) result.times = [0, Math.max(...time)];
      else result.times = [0, 0];

      const prices = await meals.map((item) => item.price);
      // if (prices.length >= 2) result.push({ prices: [Math.min(...prices), Math.max(...prices)] })
      // else result.push({ prices: prices[0] })
      if (prices.length >= 2)
        result.prices = [Math.min(...prices), Math.max(...prices)];
      else result.prices = prices[0];
  ...
  async placeEditAlbumService(id, img, placeId) {
    try {
      if (!img) {
        return Response.BadRequest("Image required!", []);
      }
      const album = await Models.PlaceImages.update(
        { img: img },
  ...
  async placeEditScheduleService(body, placeId) {
    try {
      ...
      for (const item in body)
        if (item && item !== "id") obj[item] = body[item];
